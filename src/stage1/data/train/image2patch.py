...
def image2patch(image_path,patch_num,patch_size,patch_threshold,train_patch_dir,train_groundtruth_dir,train_mask_dir,training=True,show=True):
    ...
    img_rgb = cv2.imread(train_groundtruth_dir+image_name+"_h.tif")
    img_gray = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)

    #groundtruth=plt.imread(train_groundtruth_dir+image_name+"_h.tif")
    groundtruth = img_gray
    groundtruth=np.where(groundtruth>0,1,0)

    #mask=plt.imread(train_mask_dir+image_name+"_training_mask.gif")
    # Chandra
    img_rgb = cv2.imread(train_mask_dir+image_name+"_h_mask.tif")
    img_gray = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)

    #mask=plt.imread(train_mask_dir+image_name+"_h_mask.tif")
    mask = img_gray
